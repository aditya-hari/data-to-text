...
all_content_uris = pickle.load(open('all_content_uris_obj.pkl', 'rb'))
prev_content = pickle.load(open('not_found_uris_object.pkl', 'rb'))

not_found_uris = {'en': {}, 'pt': {}, 'de': {}}

for lang in prev_content:
    print(lang)
    for i, entity in enumerate(prev_content[lang]):
        print(i, entity)
        time.sleep(3)
        try:
            with time_limit(3):
...
dump = open('all_content_uris_obj.pkl', 'wb')
pickle.dump(all_content_uris, dump)
dump.close()

dump_not_found_subject = open('not_found_uris_object.pkl', 'wb')
