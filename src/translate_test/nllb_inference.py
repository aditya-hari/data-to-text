...
nllb_model = AutoModelForSeq2SeqLM.from_pretrained("/scratch/nllb_all/checkpoint-15345/")
nllb_model.eval()
...
data_file = [i.strip() for i in open(f'data/processed/ga/dev_src.txt').readlines()]
ref_file = [i.strip() for i in open(f'data/processed/ga/dev_ref.txt').readlines()]
gens_actual = [i.strip() for i in open(f'data/results/eng_all_gen.txt').readlines()]
